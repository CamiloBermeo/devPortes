...
import { obtenerFechasDisponibles, obtenerHorasDisponibles, obtenerMetodosPago, crearReserva } from '../api/reservations.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function obtenerPrimerDiaMesBogota() {
    const [year, month] = obtenerAhoraBogota().fecha.split('-').map(Number);
  ...
  const estadoCalendario = {
    fechaActual: obtenerPrimerDiaMesBogota(),
  ...
  const datosCancha = {
    ...
    precio: '$80.000',
    imagen:
      'https://raw.githubusercontent.com/CamiloBermeo/devPortes/refs/heads/main/assets/img/canchas/futbol-estadio-principal.webp',
    sede: '',
    direccion: '',
    qrUbicacion: '',
    urlUbicacion: '',
  ...
  const renderCalendar = async () => {
    ...
    const hoyBogota = obtenerAhoraBogota().fecha;
  ...
  const renderHorarios = () => {
    ...
    horariosDisponiblesLista.forEach((horario) => {
      ...
      const horarioPasado = fechaReservaInput?.value ? horarioYaPaso(fechaReservaInput.value, horario.value) : false;

      if (horario.ocupado || horarioPasado) {
        horarioButton.classList.add('btn-horario-ocupado');
        horarioButton.disabled = true;
        if (horarioPasado) {
          horarioButton.title = 'Este horario ya pasó';
        }
      } else {
  ...
  const horarioYaPaso = (fecha, hora) => {
    const [horas, minutos] = hora.split(':').map(Number);
    const ahoraBogota = obtenerAhoraBogota();
    const minutosHorario = horas * 60 + minutos;

    if (fecha < ahoraBogota.fecha) return true;
    if (fecha > ahoraBogota.fecha) return false;
    return minutosHorario <= ahoraBogota.minutos;
  };
  ...
  const obtenerTotalReserva = () => {
    ...
    const precioNumerico = Number(String(datosCancha.precio).replace(/\D/g, '')) || 0;
    ...
    modalElement.querySelector('#btnAceptarPago')?.addEventListener('click', async () => {
      ...
      try {
        const resultado = await crearReserva({
          ...
          remainingPayment: Math.round(obtenerTotalReserva() * 0.2),
        });
        ...
        const esHorarioInvalido = error.status === 400 && /horarios pasados/i.test(error.message || '');
        showToast(
          esHorarioInvalido
            ? 'Ese horario ya pasó. Selecciona una hora futura.'
            : error.status === 409
              ? 'Ese horario acaba de ser reservado. Selecciona otro horario.'
              : 'Error al crear la reserva. Intenta de nuevo.',
          esHorarioInvalido || error.status === 409 ? 'advertencia' : 'error',
        );
        return;
      }
  ...
  botonesSiguiente.forEach((boton) => {
    boton.addEventListener('click', () => {
      ...
      if (pasoActual === 2) {
        ...
        const horasSeleccionadas = estadoCalendario.horaSeleccionadas;
        if (!fecha || horasSeleccionadas.length === 0) {
          showToast('Selecciona la fecha y al menos una hora para continuar.', 'error');
          return;
        }

        const horas = fecha ? horasSeleccionadas.filter((hora) => !horarioYaPaso(fecha, hora)) : [];

        if (horas.length !== horasSeleccionadas.length) {
          estadoCalendario.horaSeleccionadas = horas;
          renderHorarios();
          actualizarResumen();
          showToast('Una o más horas seleccionadas ya pasaron. Elige horarios futuros.', 'advertencia');
          return;
        }
  ...
  if (botonReiniciar) {
    botonReiniciar.addEventListener('click', () => {
      ...
      estadoCalendario.horaSeleccionadas = [];
      estadoCalendario.fechaActual = obtenerPrimerDiaMesBogota();
      renderCalendar();
      renderHorarios();
      actualizarDisponibilidadCambioCancha();
  ...
  if (!estadoCalendario.fechaSeleccionada) {
    const hoyStr = obtenerAhoraBogota().fecha;
