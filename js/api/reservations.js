...
function normalizarDatosReserva(raw) {
  const estado = String(raw.estado || raw.status || 'PENDIENTE')
    .trim()
    .toUpperCase();
  return {
    id: raw.id,
    ...
    estado,
    estadoTexto: formatearEstadoReserva(estado),
    totalPago: raw.totalPay ?? 0,
    saldoPendiente: raw.remainingPayment ?? 0,
    jugador: raw.clientName || raw.cliente || '',
    sede: raw.locationName || '',
    direccion: raw.locationAddress || '',
...
export async function obtenerHistorialReservas(signal) {
  try {
    ...
    const reservas = Array.isArray(data) ? data : [];
    return reservas
      .map(normalizarDatosReserva)
      .filter((reserva) => reserva.estado === 'COMPLETADA' || reserva.estado === 'CANCELADA');
  } catch (error) {
...
export async function registrarPagoFinal(reservationId, paymentMethodId, notes = '') {
  return apiPost(
    '/payment-method/reservations/final-payment',
    {
