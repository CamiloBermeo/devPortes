...
import { obtenerReservasPendientes, obtenerHistorialReservas, cancelarReserva } from '../api/reservations.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const token = localStorage.getItem('devportes_usuario_token');
  ...
  async function cargarEstadisticasUsuario() {
    ...
    const nivel = document.getElementById('nivelJugadorUsuario');

    try {
      const estadisticas = await obtenerEstadisticasUsuario(pageLoadController.signal);
      const totalReservas = Math.max(Number(estadisticas.totalReservations) || 0, 0);
      const horasJugadas = Math.max(Number(estadisticas.playedHours) || 0, 0);
      const nivelJugador = calcularNivelJugador(totalReservas, horasJugadas);

      if (reservas) reservas.textContent = String(totalReservas);
      if (horas) horas.textContent = `${horasJugadas}h`;
      if (favorita) favorita.textContent = estadisticas.favoriteField || 'N/A';
      if (nivel) {
        nivel.textContent = `${nivelJugador.nombre} · ${nivelJugador.puntos} pts`;
        nivel.className = `info-valor nivel-jugador ${nivelJugador.clase}`;
        nivel.title = `${totalReservas} reservas y ${horasJugadas} horas jugadas`;
      }
      ...
      if (nivel) {
        nivel.textContent = 'Sin datos';
        nivel.className = 'info-valor nivel-jugador nivel-sin-datos';
  ...
  function calcularNivelJugador(totalReservas, horasJugadas) {
    const puntos = Math.round(totalReservas * 10 + horasJugadas * 2);
    if (puntos >= 220) return { nombre: 'VIP', clase: 'nivel-vip', puntos };
    if (puntos >= 120) return { nombre: 'Oro', clase: 'nivel-oro', puntos };
    if (puntos >= 50) return { nombre: 'Plata', clase: 'nivel-plata', puntos };
    return { nombre: 'Bronce', clase: 'nivel-bronce', puntos };
  }
  ...
  inputFotoPerfil?.addEventListener('change', async () => {
    ...
    try {
      ...
      if (respuesta?.token) localStorage.setItem('devportes_usuario_token', respuesta.token);
  ...
  function renderizarHistorial(reservas) {
    ...
    tbody.innerHTML = reservas
      .map((r) => {
        const claseEstado = r.estadoTexto.toLowerCase().replace(/\s/g, '_');
        return `
  ...
  function openModal() {
    ...
    const btnGuardarPerfil = formEditarPerfil?.querySelector('.btn-modal-guardar');
  ...
  function validarNombreEdit() {
    ...
    if (v.length < 2 || v.length > 80) {
      marcarInvalido(inputNombre, 'Entre 2 y 80 caracteres');
      return false;
    }
  ...
  function validarEmailEdit() {
    ...
    if (v === '') {
      marcarInvalido(inputEmail, 'El correo es obligatorio');
      return false;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)) {
      marcarInvalido(inputEmail, 'Ingresa un correo válido');
      return false;
    }
  ...
  function validarTelefonoEdit() {
    ...
    if (v === '') {
      marcarInvalido(inputTelefono, 'El teléfono es requerido');
      return false;
    }
    if (!/^\d{10}$/.test(v)) {
      marcarInvalido(inputTelefono, 'Debe tener 10 dígitos');
      return false;
    }
  ...
  function validarCedulaEdit() {
    ...
    if (v === '') {
      marcarInvalido(inputCedula, 'La cédula es requerida');
      return false;
    }
    if (!/^\d{3,12}$/.test(v)) {
      marcarInvalido(inputCedula, 'Solo números (3-12 dígitos)');
      return false;
    }
  ...
  if (btnCerrarSesion) {
    btnCerrarSesion.addEventListener('click', async () => {
      const confirmar = await showConfirm('¿Estás seguro de que deseas cerrar sesión?', 'Cerrar sesión');
      if (confirmar) {
        cerrarSesion();
      }
    });
  ...
  if (formEditarPerfil) {
    formEditarPerfil.addEventListener('submit', async (evento) => {
      ...
      btnGuardarPerfil?.classList.add('loading');
      if (btnGuardarPerfil) {
        btnGuardarPerfil.disabled = true;
        btnGuardarPerfil.setAttribute('aria-busy', 'true');
      }

      try {
        const respuesta = await apiPut(
          '/auth/profile',
          {
            name: nombre,
            email,
        ...
        if (respuesta?.token) {
          localStorage.setItem('devportes_usuario_token', respuesta.token);
        }
        ...
        btnGuardarPerfil?.classList.remove('loading');
        if (btnGuardarPerfil) {
          btnGuardarPerfil.disabled = false;
          btnGuardarPerfil.removeAttribute('aria-busy');
        }
      }
    });
  }
  ...
  function openDetalle(tr, idReserva) {
    ...
    const pagoLabel = document.getElementById('detallePagoLabel');
    const pagoEl = document.getElementById('detallePago');
    const cancelacionEl = document.getElementById('detalleCancelacion');
    const esCancelada = estadoNorm === 'cancelada';

    pagoLabel.innerHTML = '<i class="bi bi-credit-card-fill"></i> Pago';
    pagoEl.textContent = esCancelada ? 'No aplica por cancelación' : reservaDataPago(tr);
    cancelacionEl.hidden = !esCancelada;
